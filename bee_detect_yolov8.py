        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    # 遍历每个边界框，获取其坐标点 (xyxy)、置信度 (conf) 和类别标签 (cls)。
                    # 获取原始图像的高度和宽度，将边界框坐标转换为中心宽高表示 (x_c, y_c, bbox_w, bbox_h)，并存储到列表 bbox_xywh 中。
                    # 将置信度和类别标签分别存储到列表 confs 和 clas 中。
                    # 将原始的边界框坐标存储到列表 xy 中。
        return np.array(bbox_xywh), confs, clas, xy

    
if __name__ == '__main__':
    person_detect = Bee_detect(source='/media/zengwb/PC/p/ReID-dataset/channel1/1.mp4')
    with torch.no_grad():
