...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default='video/demo_2.mp4', help='file/dir/URL/glob, 0 for webcam')
    ...
    if os.path.isdir(opt.source):
        entries = sorted(os.listdir(opt.source))
        for entry in entries:
            file_path = os.path.join(opt.source, entry)
            if os.path.isfile(file_path):
                print(f'Processing file: {file_path}')
            else:
                print(f'Skipping directory:{file_path}')
    else:
        print(f'Skipping directory:{opt.source}')
