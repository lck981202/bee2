...
def run(args):
    ...
    for frame_idx, batch in enumerate(predictor.dataset):
        ...
        for i in range(n):
            ...
            # 实时输出界面
            # label = "track_ID_MAX:{}".format(str(total_track))
            # predictor.put_text_to_video(label, (200, 300))
            # label_in = "in_count:{}".format(str(in_count))
            # predictor.put_text_to_video(label_in, (200, 360))
            # label_out = "out_count:{}".format(str(out_count))
            # predictor.put_text_to_video(label_out, (200, 420))
            # if last_track_id >= 0:


            # display an image in a window using OpenCV imshow() 直接关闭就可以不再显示
...
def parse_opt():
    ...
    parser.add_argument('--yolo-model', type=Path, default=WEIGHTS / 'v8n_250.pt', help='model.pt path(s)')
    ...
    # 跟踪器：ocsort deepocsort botsort bytetrack strongsort
    # E:/192.168.1.133/04/09/video-19-00-03.avi
    parser.add_argument('--source', type=str, default='E:/192.168.1.133/05/16/video-19-00-03.avi', help='file/dir/URL/glob, 0 for webcam')
