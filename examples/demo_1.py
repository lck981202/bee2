...
def run(args):
    ...
    for frame_idx, batch in enumerate(predictor.dataset):
        ...
        for i in range(n):
            ...
            #设置检测区域
            line_1 = predictor.line_set(0.59, 0.3, 1.25, 0.3) # 1/2长度
            line_2 = predictor.line_set(0.59, 0.316, 1.25, 0.316) # 1/4长度
            line_3 = predictor.line_set(0.59, 0.33, 1.25, 0.33) # 门前
            line_5 = predictor.line_set(0.59, 0.27, 1.25, 0.27) # 单倍长度
            line_6 = predictor.line_set(0.59, 0.21, 1.25, 0.21) # 二倍长度
            ...
            # predictor.line_show(line_6, 255, 0, 255, 3)
            ...
            for track in track_bees:
                ...
                # 判断1线进入线进入情况
                if intersect(midpoint, previous_midpoint, line_2[0], line_2[1]) and track_id not in already_counted:
                    class_counter[track_cls] += 1
                    total_counter += 1
                    last_track_id = track_id
                    # 经过线跳红
                    predictor.line_show(line_2, 255, 0, 0, 2)  # 通过线闪动且变粗变红
            ...
            # 实时输出界面
            label = "total_track:{}".format(str(total_track))
            predictor.put_text_to_video(label, (250, 400))
            label_in = "in_count:{}".format(str(in_count))
            predictor.put_text_to_video(label_in, (250, 440))
            label_out = "out_count:{}".format(str(out_count))
            predictor.put_text_to_video(label_out, (250, 460))
            # if last_track_id >= 0:
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default='video/in_out_1.mp4', help='file/dir/URL/glob, 0 for webcam')
